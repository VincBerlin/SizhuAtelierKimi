  ...
  return (
    <main style={{ background: C.bg, minHeight: '60vh' }}>
      <div style={{ maxWidth: CONTAINER, margin: '0 auto', padding: '48px 32px 64px' }}>
        <div style={{ fontFamily: FONT_SANS, fontSize: 12, letterSpacing: '0.28em', textTransform: 'uppercase', color: C.accent, marginBottom: 12 }}>{t('pages.blogEyebrow')}</div>
        <h1 style={{ fontFamily: FONT_SERIF, fontWeight: 400, fontSize: 'clamp(34px,5vw,52px)', color: C.ink, margin: '0 0 14px', lineHeight: 1.08 }}>{t('pages.blogTitle')}</h1>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px,1fr))', gap: 24 }}>
          {articles.map((a) => (
            ...
            >
              ...
              <p style={{ fontFamily: FONT_SANS, fontSize: 14, lineHeight: 1.6, color: C.textMuted, margin: 0 }}>{t(`content.articles.${a.id}.excerpt`)}</p>
              <span style={{ fontFamily: FONT_SANS, fontSize: 13, color: C.ink, fontWeight: 500, marginTop: 4 }}>{t('pages.blogRead')}</span>
            </Link>
          ))}
        </div>
      </div>
