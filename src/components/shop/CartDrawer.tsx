...
export default function CartDrawer() {
  const { cart, cartOpen, closeCart, subtotal, shipCost, reached, remaining, setQty, removeLine, addAddon } = useShopStore()
  const navigate = useNavigate()
  const open = cartOpen
  const hasCart = cart.length > 0
  const shipPct = Math.min(100, (subtotal / FREE_SHIP_THRESHOLD) * 100) + '%'
  const shipMessage = reached ? '✓ Glückwunsch — du erhältst kostenlosen Versand!' : 'Dir fehlen nur noch ' + euro(remaining) + ' bis zum kostenlosen Versand!'
    ...
    <>
      ...
      <aside style={{ position: 'fixed', top: 0, right: 0, height: '100%', width: 'min(440px, 100vw)', background: C.bg, zIndex: 51, boxShadow: '-20px 0 50px -20px rgba(28,24,18,0.4)', transform: `translateX(${open ? '0' : '100%'})`, transition: 'transform .4s cubic-bezier(0.4,0,0.2,1)', display: open ? 'flex' : 'none', flexDirection: 'column' }}>
        <div style={{ padding: '22px 24px', borderBottom: `1px solid ${C.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <span style={{ fontFamily: FONT_SERIF, fontSize: 24 }}>Warenkorb</span>
        ...
        <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: '8px 24px' }}>
          {!hasCart && (
            <div style={{ textAlign: 'center', padding: '60px 20px', color: C.textMuted3 }}>
              <div style={{ fontFamily: FONT_SERIF, fontSize: 24, color: C.textMuted, marginBottom: 8 }}>Noch leer</div>
              <p style={{ fontSize: 14, margin: '0 0 20px' }}>Gestalte dein erstes BaZi-Poster.</p>
              <button onClick={goShop} style={{ background: C.ink, color: C.bg, border: 'none', cursor: 'pointer', padding: '12px 22px', borderRadius: 999, fontSize: 14, fontFamily: FONT_SANS }}>Zur Kollektion</button>
            </div>
          )}
            ...
            <div key={i.key} style={{ display: 'flex', gap: 14, padding: '18px 0', borderBottom: `1px solid ${C.border}` }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8 }}>
                  ...
                  <button onClick={() => removeLine(i.key)} className="transition-colors hover:text-[#C0492E]" style={{ background: 'none', border: 'none', cursor: 'pointer', color: C.strike, fontSize: 13, flexShrink: 0 }}>Entfernen</button>
                </div>
          ...
          {hasCart && (
            <div style={{ padding: '18px 0 8px' }}>
              <div style={{ fontSize: 12, letterSpacing: '0.06em', textTransform: 'uppercase', color: C.textMuted3, marginBottom: 12 }}>Dazu passt perfekt</div>
                ...
                {addons.slice(0, 3).map((a) => (
                  <div key={a.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: 10, border: `1px solid ${C.border}`, borderRadius: 10, background: '#fff' }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: 11, color: C.textMuted2 }}>{a.note}</div>
                    </div>
        ...
        {hasCart && (
          <div style={{ borderTop: `1px solid ${C.border}`, padding: '18px 24px 22px', background: '#fff' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 4 }}>
              <span style={{ fontSize: 14, color: C.textMuted }}>Zwischensumme</span>
              <span style={{ fontSize: 20, fontWeight: 700 }}>{euro(subtotal)}</span>
            </div>
            <div style={{ fontSize: 12, color: C.textMuted2, marginBottom: 14 }}>{shipText} · inkl. MwSt.</div>
            <button onClick={goCheckout} className="transition-[filter] hover:brightness-110" style={{ width: '100%', background: C.accent, color: '#fff', border: 'none', cursor: 'pointer', padding: 16, borderRadius: 12, fontSize: 16, fontWeight: 600, fontFamily: FONT_SANS, boxShadow: '0 12px 24px -12px rgba(192,73,46,0.6)' }}>Zur Kasse</button>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 14, marginTop: 14, fontSize: 11, color: C.textMuted3 }}>
              <span style={{ fontWeight: 600, color: C.textMuted }}>PayPal</span><span style={{ fontWeight: 600, color: C.textMuted }}> Pay</span><span style={{ fontWeight: 600, color: C.textMuted }}>G Pay</span><span>· SSL gesichert</span>
            </div>
          </div>
        )}
      </aside>
