  ...
  return (
    <div onClick={go} style={{ cursor: 'pointer', display: 'flex', flexDirection: 'column' }}>
      {personalizable ? (
        <PosterScene poster={product.poster} hover aspect="3 / 4">
          {COMMERCE_ENABLED && hasAnchor && (
            <span style={{ position: 'absolute', top: 14, left: 14, background: C.accent, color: '#fff', fontSize: 11, fontWeight: 600, letterSpacing: '0.04em', padding: '5px 10px', borderRadius: 4, zIndex: 2 }}>−{euro((product.anchor as number) - product.price)}</span>
          )}
        </PosterScene>
      ) : (
        <div style={{ position: 'relative', width: '100%', aspectRatio: '3 / 4', overflow: 'hidden', border: `1px solid ${C.border}`, background: C.surfaceWarm }}>
      ...
      <div style={{ padding: '16px 2px 0' }}>
        ...
        {personalizable && <p style={{ fontSize: 12.5, color: C.textMuted2, margin: '0 0 10px', lineHeight: 1.45 }}>{t('card.personalLine')}</p>}
        ...
        <div style={{ marginTop: 12, fontFamily: FONT_SANS, fontSize: 13, fontWeight: 600, color: C.accent }}>{personalizable ? t('card.personalize') : t('card.shop')} →</div>
      </div>
    </div>
  )
}
