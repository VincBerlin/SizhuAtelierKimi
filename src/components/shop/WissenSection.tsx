    ...
    <section id="wissen" style={{ background: C.surfaceWarm, borderTop: `1px solid ${C.border}`, marginTop: 24 }}>
      <div style={{ maxWidth: CONTAINER, margin: '0 auto', padding: '56px 32px' }}>
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12, marginBottom: 30 }}>
          <div>
            <div style={{ fontSize: 12, letterSpacing: '0.28em', textTransform: 'uppercase', color: C.accent, marginBottom: 10 }}>Wissen</div>
            <h2 style={{ fontFamily: FONT_SERIF, fontWeight: 400, fontSize: 32, margin: 0 }}>Was hinter BaZi steckt</h2>
          </div>
          <p style={{ fontSize: 13, color: C.textMuted2, maxWidth: 320, margin: 0 }}>Hintergründe für deine Beratung und für alle, die ihr Poster verstehen möchten.</p>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 24 }}>
          {articles.map((a) => (
            ...
            >
              <span style={{ fontSize: 11, letterSpacing: '0.18em', textTransform: 'uppercase', color: C.accent }}>{a.tag}</span>
              <h3 style={{ fontFamily: FONT_SERIF, fontWeight: 500, fontSize: 22, margin: 0, lineHeight: 1.2, color: C.ink }}>{a.title}</h3>
              <p style={{ fontSize: 14, lineHeight: 1.6, color: C.textMuted, margin: 0 }}>{a.excerpt}</p>
              <span style={{ fontSize: 13, color: C.ink, fontWeight: 500, marginTop: 4 }}>Weiterlesen →</span>
            </button>
          ))}
        </div>
      </div>
