...
export default function ArticleOverlay() {
  ...
  const active = articles.find((a) => a.id === articleId)
    ...
    <>
      ...
      <aside style={{ position: 'fixed', top: 0, right: 0, height: '100%', width: 'min(620px, 100vw)', background: C.bg, zIndex: 61, boxShadow: '-20px 0 50px -20px rgba(28,24,18,0.4)', transform: `translateX(${open ? '0' : '100%'})`, transition: 'transform .4s cubic-bezier(0.4,0,0.2,1)', display: open ? 'flex' : 'none', flexDirection: 'column' }}>
        <div style={{ padding: '20px 28px', borderBottom: `1px solid ${C.border}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span style={{ fontSize: 12, letterSpacing: '0.18em', textTransform: 'uppercase', color: C.accent }}>{active?.tag}</span>
        ...
        <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: '32px 40px 60px' }}>
          <h1 style={{ fontFamily: FONT_SERIF, fontWeight: 500, fontSize: 34, lineHeight: 1.12, margin: '0 0 8px' }}>{active?.title}</h1>
          <div style={{ fontSize: 13, color: C.textMuted3, marginBottom: 24 }}>{active?.meta}</div>
          ...
          <div style={{ marginTop: 24, padding: 22, background: C.surfaceWarm, borderRadius: 12 }}>
            <div style={{ fontFamily: FONT_SERIF, fontSize: 20, marginBottom: 8 }}>Bereit für dein eigenes Chart?</div>
            <button onClick={() => { closeArticle(); navigate('/produkt/1'); window.scrollTo(0, 0) }} className="transition-[filter] hover:brightness-110" style={{ background: C.accent, color: '#fff', border: 'none', cursor: 'pointer', padding: '12px 22px', borderRadius: 999, fontSize: 14, fontFamily: FONT_SANS }}>Poster gestalten →</button>
          </div>
        </div>
      </aside>
    </>
