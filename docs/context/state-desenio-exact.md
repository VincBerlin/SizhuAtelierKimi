...
| M16 | nav→filter deep-link | done | ✅ cavecrew (caught+fixed size-as-fake-filter) | ✅ tsc 0 + hub smoke 13/0 (+ wholebuild 51/0) | ✅ scope 6 | ✅ committed 932d148 | `Kollektion.tsx` consumes ?style/?room → pre-filters all-posters grid + active-facet banner + reset; collection cards hidden while faceted; ?size disclosed non-narrowing (M13); new `exact-nav-filter-deeplink.test.tsx`; taxonomy docstrings flipped PARTIAL→wired; i18n×4 `coll.filter` |
| M17 | Collection URL-sync | done | ✅ cavecrew (No issues) | ✅ tsc -b 0 + smoke 9/0 | ✅ scope 4 | ✅ committed 2a3e990 | `Collection.tsx` facet matrix URL-driven (useSearchParams; ?style/?room/?price source-of-truth); deep-link honored on load, chip→URL, reset clears, empty-state on unknown; slug-change keeps URL facets (only local resets); new `exact-collection-url-sync.test.tsx`; extends M16 hub deep-link into the collection template |
| M18 | Accessibility/Keyboard | done | ✅ cavecrew (caught+fixed skip-link focus bug) | ✅ tsc -b 0 + a11y smoke 12/0 | ✅ scope 7 | ✅ committed 841342e | skip link + focusable #main-content + route-change focus (App); mega-menu aria-controls→panel id (Navbar); FilterRow role=group+aria-label, chips aria-pressed (Collection); i18n×4 a11y.skipToContent; new `exact-a11y.test.tsx`; quality pass (not a PRD REQ) |
