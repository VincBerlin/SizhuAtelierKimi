...
| **M18** Accessibility/Keyboard (quality; no specific PRD REQ) | — | A11y/keyboard pass on the new nav/filter surfaces | App: localized skip link → focusable `#main-content` region + focus moves there on route change (keyboard/SR users land on content, not stranded in chrome); Navbar: mega-menu trigger `aria-controls`→panel `id` (aria-haspopup/expanded + Escape already present); Collection: FilterRow `role="group"`+aria-label, facet chips keep aria-pressed; i18n×4 `a11y.skipToContent`; `exact-a11y.test.tsx`; renderToStaticMarkup smoke 12/0 in-sandbox; `tsc -b` 0 | **TEST-GREEN** (jsdom on dev machine) |
