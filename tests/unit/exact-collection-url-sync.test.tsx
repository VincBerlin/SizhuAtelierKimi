 ...
 * /collections/bazi-posters?style=minimal must pre-apply the filter on load; a chip
...
describe('M17 — a deep-linked filter is honored on load', () => {
  it('/collections/bazi-posters?style=minimal pre-applies the style filter (grid + active chip)', async () => {
    renderAt('/collections/bazi-posters?style=minimal')
    await page()
    await waitFor(() => expect(cardIds()).toEqual([2, 6])) // only minimal-family bazi posters
...
describe('M17 — a chip click writes the URL (shareable / back-navigable)', () => {
  it('selecting a style chip adds ?style=… to the URL and narrows the grid', async () => {
    renderAt('/collections/bazi-posters')
    await page()
    ...
    await waitFor(() => expect(cardIds()).toEqual([2, 6]))
  })
})
...
describe('M17 — an impossible deep-link yields an honest empty state, not the full set', () => {
  it('?style=nonexistent shows the empty state and no cards', async () => {
    renderAt('/collections/bazi-posters?style=nonexistent')
    await page()
